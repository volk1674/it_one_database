...
BEGIN
    ...
    WITH cp AS (SELECT c.item, PERCENTILE_CONT(v_pf_customer) WITHIN GROUP (ORDER BY price_per_unit) s_price
                FROM customers c
                GROUP BY c.item),
         vp AS (SELECT v.item, PERCENTILE_CONT(v_pf_vendor) WITHIN GROUP (ORDER BY price_per_unit DESC) p_price
                FROM vendors v
                GROUP BY v.item)
    SELECT cp.item,
...
BEGIN
    ...
    INSERT INTO actions.offers(contractor, quantity)
        (WITH delivery AS (SELECT v_island, SUM(quantity) AS total FROM my_ship_delivery GROUP BY v_island)
         ...
         WHERE COALESCE(s.quantity, 0) < 700 + COALESCE(delivery.total, 0));
EXCEPTION
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS
            v_error_message = MESSAGE_TEXT;
...
BEGIN
    ...
    FOR rec IN (SELECT *
                ...
                WHERE id IN (SELECT MIN(id)
                             FROM my_ship_delivery d
                                      JOIN world.moving_ships ms ON ms.destination = d.v_island AND ms.ship = d.ship and (ms.arrives_at - r_global.game_time) < 30
                             WHERE d.state = 0
                             GROUP BY d.ship))
        LOOP
            INSERT INTO actions.transfers(ship, item, quantity, direction)
    ...
    FOR rec IN (SELECT DISTINCT d.ship, d.item, c.quantity
                FROM my_ship_delivery d
                         -- корабль припаркован рядом с островом назначения
                         JOIN world.moving_ships ms ON ms.destination = d.c_island AND ms.ship = d.ship and (ms.arrives_at - r_global.game_time) < 30
                         JOIN world.cargo c ON c.ship = d.ship AND c.item = d.item
                WHERE d.state = 3)
        LOOP
            INSERT INTO actions.transfers(ship, item, quantity, direction)
...
BEGIN
    ...
    INSERT INTO actions.wait(until) VALUES (r_global.game_time + 1);
